    ...
    <div className="w-full max-w-2xl space-y-4">
      ...
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-6">
        ...
        <div className="flex justify-center gap-4 text-sm">
          <button onClick={() => openWords("known")} className={`flex items-center gap-1.5 px-3 py-1 rounded-lg transition-colors ${panel === "known" ? "bg-emerald-500/20 text-emerald-300" : "text-slate-400 hover:text-emerald-300"}`}>
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 inline-block" />Known ({knownCount}) ↗
          </button>
          <button onClick={() => openWords("learning")} className={`flex items-center gap-1.5 px-3 py-1 rounded-lg transition-colors ${panel === "learning" ? "bg-amber-500/20 text-amber-300" : "text-slate-400 hover:text-amber-300"}`}>
            <span className="w-2.5 h-2.5 rounded-full bg-amber-400 inline-block" />Learning ({learningCount}) ↗
          </button>
          <span className="flex items-center gap-1.5 text-slate-500">
            <span className="w-2.5 h-2.5 rounded-full bg-slate-600 inline-block" />New ({dontKnowCount})
          </span>
        </div>
      </div>
